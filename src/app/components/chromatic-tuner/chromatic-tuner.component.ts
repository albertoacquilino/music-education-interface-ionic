  ...
  constructor(
    ...
    this.note$ = this.pitch$.pipe(
      map((pitch) => {
        ...
        console.log(noteDist[0]);
        //Calculate the cents from the frequency of closest note and detected pitch 
        let cents = (pitch <= 0) ? (0) : 1200 * Math.log2(pitch / NOTES[noteDist[0].note].freq);
